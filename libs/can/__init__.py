...
class CANSocket:
    def __init__(self, interface='can0', devices_id=[0x001], serial_port=None, device=None, bitrate=1000000, reset = True):
        ...
        if serial_port or device == 'can_hacker':
            self.can_hacker_init(port=serial_port, baud_rate=baudrate)
    ...
    def recive_frame(self):
        ...
        try:
            ...
            frame, addr = self.socket.recvfrom(recvBytes)
...
class CANDevice:
    ...
    def __del__(self):
        self.command = self.empty
        self.reply = self.empty
        print('CAND Device was destructed')

    def send_command(self, command):
        # print("Try to send command")
    ...
    def _send_raw_command(self, p_des, v_des, kp, kd, tau_ff):
        ...
        try:
            self.send_command(self._cmd_bytes.tobytes())
            waitOhneSleep(dt_sleep)
            can_id, can_dlc, data = self.reciver()
            return data
        except Exception as e:
